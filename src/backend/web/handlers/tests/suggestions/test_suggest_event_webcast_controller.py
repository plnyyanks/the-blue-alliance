...
def test_submit_no_event(login_user, web_client: Client) -> None:
    resp = web_client.post("/suggest/event/webcast", data={}, follow_redirects=True)
    assert resp.status_code == 404


def test_submit_empty_form(login_user, web_client: Client) -> None:
...
def test_submit_bad_url(login_user, web_client: Client) -> None:
    resp = web_client.post(
...
def test_submit_tba_url(login_user, web_client: Client) -> None:
    resp = web_client.post(
        "/suggest/event/webcast",
